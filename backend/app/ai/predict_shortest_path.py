...
def a_star_algorithm_with_alternatives(routes, start, end):
    ...
    def reconstruct_path(node):
        ...
        while node:
            ...
            if node["parent"]:
                via_list.extend(node["route"].get("via", []))
                total_distance += node["route"]["distance"]
                total_fee += node["route"].get("fare", 0)  # Add fee from the route
            node = node["parent"]

        return path[::-1], list(set(via_list)), total_distance, total_fee
    ...
    while open_list:
        ...
        # If we've reached the end, reconstruct the path
        if current_node["location"] == end:
            path, via_list, distance, fee = reconstruct_path(current_node)
            found_routes.append({
                "path": path,
                "distance": distance,
                "fee": fee,  # Include the fee in the route
                "via": via_list
            })
            continue
    ...
    if found_routes:
        found_routes.sort(key=lambda x: x["distance"])
    return found_routes if found_routes else [{"path": ["No Path Found"], "distance": 0, "fee": 0, "via": []}]
