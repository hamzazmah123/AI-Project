...
function DriverAssign() {
  ...
  const fetchDrivers = async () => {
    try {
      const { data } = await axios.get(
        "http://localhost:5000/auth/drivers/unassigned"
      );
      setDrivers(data);
    } catch (error) {
      console.error("Error fetching drivers:", error);
  ...
  const handleAssign = async () => {
    if (selectedVehicle && selectedDriver) {
      try {
        await axios.post("http://localhost:5000/auth/assignments", {
          vehicle_id: vehicles.find((v) => v.name === selectedVehicle)._id,
          driver_id: drivers.find((d) => d.name === selectedDriver)._id,
          name: selectedDriver,
        });

        await axios.post("http://localhost:5000/auth/vehicle-assignments", {
          vehicle_id: vehicles.find((v) => v.name === selectedVehicle)._id
        });

        alert(`Assigned ${selectedDriver} to ${selectedVehicle}`);
        fetchDrivers(); // Refresh the drivers to exclude the newly assigned one
        setSelectedVehicle("");
        setSelectedDriver("");
      } catch (error) {
        console.error("Error assigning driver:", error);
